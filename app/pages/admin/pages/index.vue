...
async function fetchPages() {
  ...
  loading.value = true
  try {
    const data = await $fetch(`/api/pages?companyId=${company.value?.id}`)
...
async function deletePage(pageId: string) {
  ...
  try {
    await $fetch(`/api/pages/${pageId}?companyId=${company.value?.id}`, {
      method: 'DELETE'
    })
    toast.add({
      title: 'Page deleted',
...
function getPageActions(row: any): DropdownMenuItem[][] {
  return [[{
    ...
    to: `/admin/pages/${row.original.id}/analytics`
  }], [{
    label: 'Duplicate',
    icon: 'i-lucide-copy',
